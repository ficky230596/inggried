...
if uploaded_file is not None:
    ...
    # Prediksi dengan Regresi Polinomial (Polynomial Regression Degree 2)
    ...
    for kolom in ["Belanja_Pegawai", "Belanja_Barang_Jasa", "Belanja_Modal", "Total_Belanja", "PAD"]:
        y = df[kolom].values
        model = LinearRegression()
        model.fit(X_poly, y)
        X_prediksi_poly = poly.transform(tahun_prediksi)
        pred = model.predict(X_prediksi_poly)
    ...
    if menu == "📑 Data Anggaran":
        st.subheader("📑 Data Anggaran (2019–2024)")
        ...
        kategori = st.selectbox(
            ...
            ["Belanja_Pegawai", "Belanja_Barang_Jasa", "Belanja_Modal", "Total_Belanja", "PAD"],
        )

        # Grafik isi kecil
        fig, ax = plt.subplots(figsize=(6,3))
        ax.plot(df["Tahun"], df[kategori], marker="o", markersize=4, linewidth=1, label=kategori)
        ax.set_title(f"Pola {kategori} (2019–2024)", fontsize=9)
        ...
        st.subheader("📏 Evaluasi Algoritma Regresi Polinomial")
        ...
        with st.expander("Klik untuk melihat grafik tren"):
            ...
            for kolom in kategori:
                ax2.plot(df["Tahun"], df[kolom], marker="o", markersize=4, linewidth=1,
                         label=f"{kolom} (Aktual)")
                ax2.plot(df_prediksi["Tahun"], df_prediksi[f"Prediksi_{kolom}"],
                         marker="x", markersize=4, linestyle="--", linewidth=1,
